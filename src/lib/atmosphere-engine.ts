...
const PRODUCT_MATRIX: Record<WeatherCondition, Record<AQIQuality, { name: string; collection: string }>> = {
  clear: {
...
const WEATHER_FEELS: Record<WeatherCondition, string[]> = {
  ...
  'partly-cloudy': ['softened by passing cloud', 'diffused and gentle', 'intermittently bright'],
  overcast: ['muted and even', 'nordic in its stillness', 'hushed beneath grey'],
  ...
  storm: ['charged and volatile', 'electric at the edges', 'dramatically atmospheric'],
...
export function getMockWeather(): WeatherData {
  const conditions: WeatherCondition[] = ['clear', 'partly-cloudy', 'drizzle', 'rain', 'overcast']
...
export function generateRecommendation(ctx: AtmosphericContext): AtmosphericRecommendation {
  ...
  const product = PRODUCT_MATRIX[ctx.weather.condition]?.[ctx.aqi.quality]
    ?? { name: 'The Foundation Blend', collection: 'Daily Edition' }
...
function buildPairingNotes(ctx: AtmosphericContext, mood: MoodDescriptor): string[] {
  const notes: string[] = []

  if (ctx.weather.temperature < 8) {
    notes.push('Best experienced through an open window with a warm beverage nearby')
  } else if (ctx.weather.temperature > 22) {
    notes.push('Allows full atmospheric expansion in warmer ambient conditions')
  } else {
  ...
  if (ctx.weather.condition === 'rain' || ctx.weather.condition === 'drizzle') {
    notes.push('The precipitation contributes a petrichor undertone — a rare natural accord')
  } else if (ctx.weather.condition === 'snow') {
    notes.push('Snow compression creates a unique pressure accord found nowhere else')
  } else if (ctx.weather.condition === 'clear') {
    notes.push(`Clarity index: ${ctx.aqi.index} AQI — within our recommended range for single-origin profiles`)
  }
